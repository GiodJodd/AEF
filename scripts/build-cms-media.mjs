...
async function main() {
  ...
  try {
    ...
    console.warn(`No ${FILMS_DIR} — writing empty CMS media map.`);
    await fs.writeFile(OUT_DATA_FILE, emitTs({}));
    return;
  }
  ...
  console.log(
    count > 0
      ...
      : "No CMS film imagery found — wrote empty map (films use PROJECT_MEDIA).",
  );
}
